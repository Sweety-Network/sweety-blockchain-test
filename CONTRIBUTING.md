...
Welcome to the sweety-blockchain project!
We are happy that you are taking a look at the code for Sweety, a proof of space and time cryptocurrency.

A lot of fascinating new cryptography and blockchain concepts are used and implemented here.
This repo includes the code for the Sweety full node, farmer, and timelord (in sweety folder), which are all written in python.
It also includes a verifiable delay function implementation that it imports from the [sweetyvdf repo](https://github.com/Sweety-Network/sweetyvdf) (in c/c++), and a proof of space implementation that it imports from the [sweetypos repo](https://github.com/Sweety-Network/sweetypos). BLS signatures are imported from the [bls-signatures repo](https://github.com/Sweety-Network/bls-signatures) as blspy. There is an additional dependency on the [sweetybip158 repo](https://github.com/Sweety-Network/sweetybip158). For major platforms, binary and source wheels are shipped to PyPI from each dependent repo. Then sweety-blockchain can pip install those from PyPI or they can be prepackaged as is done for the Windows installer. On unsupported platforms, pip will fall back to the source distributions, to be compiled locally.

If you want to learn more about this project, read the [wiki](https://github.com/Sweety-Network/sweety-blockchain/wiki), or check out the [green paper](https://www.sweety.net/assets/SweetyGreenPaper.pdf).
...
Contact any of the team members on [Keybase](https://keybase.io/team/sweety_network.public), which we use as the main communication method. You can also comment on any Trello card.

We ask that external contributors create a fork of the `main` branch for any feature work they wish to take on.

Members of the Sweety organization may create feature branches from the `main` branch.

In the event an emergency fix is required for the release version of Sweety, members of the Sweety organization will create a feature branch from the current release branch `1.0.0`.
...
4. Sweety Network's nodes on the public testnet are running the latest version `x.y.z`.
...
. ./activate
pip install ".[dev]"
black sweety tests && mypy sweety tests && flake8 sweety tests
...
We provide a [pre-commit configuration](https://github.com/Sweety-Network/sweety-blockchain/blob/main/.pre-commit-config.yaml) which triggers several useful
...
7. Preferences > Settings > mypy > Targets: set to `./sweety` and `./tests`
...
6. Install these linters https://github.com/Sweety-Network/sweety-blockchain/tree/main/.github/linters

## Testnets and review environments

With the launch of `1.0.0` we will begin running an official `testnet`.
Prior to the release of `1.1.0` there will be two running test nets. `testnet` and `transaction-beta-testnet`. The `transaction-beta-testnet` testnet will be a beta of the pending 1.1 release, which will enable transactions on the sweety blockchain.
...
Sweety organization members have been granted CI access to deploy `beta testnets`.
If you are not a Sweety organization member, you can enquire about deploying a `beta testnet` in the public dev Keybase channel.

## Submit changes

To propose changes, please make a pull request to the `main` branch. See Branching Strategy above.

To propose changes for the production releases of Sweety, please make a pull request to the latest release branch.
