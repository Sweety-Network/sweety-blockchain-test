...
if [ -z "$VIRTUAL_ENV" ]; then
  echo "This requires the sweety python virtual environment."
...
THE_PATH=$(python -c 'import pkg_resources; print( pkg_resources.get_distribution("sweetyvdf").location)' 2>/dev/null)/vdf_client
SWEETYVDF_VERSION=$(python -c 'from setup import dependencies; t = [_ for _ in dependencies if _.startswith("sweetyvdf")][0]; print(t)')
...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		echo "Installing sweetyvdf from source on Ubuntu/Debian"
		...
		echo venv/bin/python -m pip install --force --no-binary sweetyvdf "$SWEETYVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary sweetyvdf "$SWEETYVDF_VERSION"
		symlink_vdf_bench "$PYTHON_VERSION"
	elif [ -e venv/bin/python ] && test $RHEL_BASED; then
		echo "Installing sweetyvdf from source on RedHat/CentOS/Fedora"
		# Install remaining needed development tools - assumes venv and prior run of install.sh
		...
		echo "Installing sweetyvdf requirement boost for MacOS."
		brew install boost
		echo "Installing sweetyvdf from source."
