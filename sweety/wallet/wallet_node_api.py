from sweety.protocols import full_node_protocol, introducer_protocol, wallet_protocol
...
from sweety.util.api_decorators import api_request, peer_required, execute_task
...
class WalletNodeAPI:
    ...
    @peer_required
    @api_request
    async def respond_removals(self, response: wallet_protocol.RespondRemovals, peer: WSSweetyConnection):
        pass

    async def reject_removals_request(self, response: wallet_protocol.RejectRemovalsRequest, peer: WSSweetyConnection):
        """
    ...
    @peer_required
    @api_request
    async def respond_additions(self, response: wallet_protocol.RespondAdditions, peer: WSSweetyConnection):
        pass
    ...
    @peer_required
    @api_request
    async def transaction_ack(self, ack: wallet_protocol.TransactionAck, peer: WSSweetyConnection):
        """
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: WSSweetyConnection
    ):
    ...
    @peer_required
    @api_request
    async def respond_peers(self, request: full_node_protocol.RespondPeers, peer: WSSweetyConnection):
        if not self.wallet_node.has_full_node():
