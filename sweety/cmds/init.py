...
def init_cmd(ctx: click.Context, create_certs: str, fix_ssl_permissions: bool, **kwargs):
    ...
    - Make a copy of your Farming Machine CA directory: ~/.sweety/[version]/config/ssl/ca
    - Shut down all sweety daemon processes with `sweety stop all -d`
    - Run `sweety init -c [directory]` on your remote harvester,
      where [directory] is the the copy of your Farming Machine CA directory
    - Get more details on remote harvester on Sweety wiki:
      https://github.com/Sweety-Network/sweety-blockchain/wiki/Farming-on-many-machines
    """
    from pathlib import Path
    from .init_funcs import init
    from sweety.cmds.passphrase_funcs import initialize_passphrase
...
if __name__ == "__main__":
    from .init_funcs import sweety_init
    from sweety.util.default_root import DEFAULT_ROOT_PATH

    sweety_init(DEFAULT_ROOT_PATH)
