...
def show_plots(root_path: Path):
    from sweety.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'sweety plots add -d [dir]' and remove with"
        + " 'sweety plots remove -d [dir]'"
        + " Scan and check plots with 'sweety plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'sweety init'")
...
def create_cmd(
    ...
    from sweety.plotting.create_plots import create_plots, resolve_plot_keys
...
def check_cmd(
    ...
    from sweety.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from sweety.plotting.util import remove_plot_directory
