...
# the new passphrase.
DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE = "$ sweety passphrase set # all the cool kids are doing it!"

MAC_KEYCHAIN_MASTER_PASSPHRASE_SERVICE = "Sweety Passphrase"
MAC_KEYCHAIN_MASTER_PASSPHRASE_USER = "Sweety Passphrase"


def check_macos_keychain_keys_present(mac_keychain: MacKeyring) -> bool:
    from keyring.credentials import SimpleCredential
    from sweety.util.keychain import default_keychain_user, default_keychain_service, get_private_key_user, MAX_KEYS
...
class KeyringWrapper:
    ...
    def _configure_backend(self) -> Union[Any, FileKeyring]:
        from sweety.util.keychain import supports_keyring_passphrase
    ...
    def _get_initial_cached_passphrase(self) -> str:
        from sweety.util.keychain import supports_os_passphrase_storage
    ...
    def confirm_migration(self) -> bool:
        ...
        if master_passphrase == DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE:
            ...
            print(
                "Would you like to set a master passphrase now? Use 'sweety passphrase set' to change the passphrase.\n"
            )

            response = prompt_yes_no("Set keyring master passphrase? (y/n) ")
            if response:
                from sweety.cmds.passphrase_funcs import prompt_for_new_passphrase
                ...
                print(
                    "Will skip setting a master passphrase. Use 'sweety passphrase set' to set the master passphrase.\n"
                )
        else:
            import colorama
    ...
    def migrate_legacy_keys(self) -> MigrationResults:
        from sweety.util.keychain import get_private_key_user, Keychain, MAX_KEYS
    ...
    def migrate_legacy_keyring_interactive(self):
        ...
        """
        from sweety.cmds.passphrase_funcs import async_update_daemon_migration_completed_if_running
