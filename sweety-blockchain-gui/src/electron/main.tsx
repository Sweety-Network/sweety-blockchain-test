...
import sweetyEnvironment from '../util/sweetyEnvironment';
import sweetyConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!sweetyEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    sweetyConfig.loadConfig('mainnet');
    ...
    const createWindow = async () => {
      ...
      if (!sweetyConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !sweetyConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (sweetyConfig.manageDaemonLifetime()) {
        sweetyEnvironment.startSweetyDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Sweety Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Sweety-Network/sweety-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Sweety-Network/sweety-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Sweety-Network/sweety-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Sweety-Network/sweety-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Sweety-Network/sweety-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://keybase.io/team/sweety_network.public');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/sweety_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Sweety Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Sweety' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Sweety Blockchain' }),
            click: () => {
              openAbout();
            },
          },
