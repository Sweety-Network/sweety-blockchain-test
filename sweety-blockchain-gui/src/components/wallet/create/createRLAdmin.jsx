...
import { AlertDialog } from '@sweety/core';
import {
  Typography,
  Button,
  Box,
...
import { sweety_to_mojo } from '../../../util/sweety';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      sweetyper_input.value === '' ||
      Number(sweetyper_input.value) === 0 ||
      !Number(sweetyper_input.value) ||
      isNaN(Number(sweetyper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const sweetyper = sweety_to_mojo(sweetyper_input.value);
    const sweetyper_value = Number.parseInt(Number(sweetyper));
    ...
    // var fee = sweety_to_mojo(fee_input.value);
