...
import { AlertDialog, Card, Flex } from '@sweety/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const hestyeck = /[\da-f]+$/gi;

    if (!hestyeck.test(ip_val) || ip_val.value === '') {
    ...
    const sweetyper_input = ip_parsed.limit;
    ...
    const sweetyper_value = Number.parseInt(Number(sweetyper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (sweety per interval):{' '}
                  {mojo_to_sweety_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const amount = sweety_to_mojo(amount_input.value);
    const fee = sweety_to_mojo(fee_input.value);
