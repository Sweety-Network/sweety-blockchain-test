...
export default function Block() {
  ...
  const poolReward = mojo_to_sweety(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_sweety(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const sweetyFees = blockRecord.fees
    ? mojo_to_sweety(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.sweetyexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.sweetyexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: sweetyFees ? `${sweetyFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    <LayoutMain title={<Trans>Block</Trans>}>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the Sweety blockchain
            </Trans>
          </Back>
        }
        action={
