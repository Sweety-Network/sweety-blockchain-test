...
const getSweetyVersion = () => {
  let version = null;
  const exePath = getExecutablePath('sweety');
  // first see if we can get a sweety exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or sweety is on the path
    try {
      version = child_process
