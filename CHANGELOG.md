...
- Added a py.typed file that allows other projects that pip install sweety-blockchain to type check using our functions with mypy.
- Added an RPC for coin records by multiple coin names.
- Enabled querying AAAA records for DNS Introducer.
- We now set the version for the GUI when doing a manual install using the install-gui.sh script. Uses a python helper to get the version of the sweety install and then converts it into proper npm format and puts that into package.json.
- Added some new class methods to the Program objects to improve ease of use.
- Added an option to sign bytes as well as UTF-8 strings, which is particularly helpful if you're writing Sweetylisp puzzles that require signatures and you want to test them without necessarily writing a whole python script for signing the relevant data.
...
- Thanks @altendky for changing the default to paginate to sweety wallet get_transactions to address cases such as piping and output redirection to a file where the command previously just hung while waiting for the user to press c for the next page.
...
- Switched IP address lookup to first use Sweety's service ip.sweety.net.
...
## 1.2.3 Sweety blockchain 2021-07-26
...
- Updated sweetypos to 1.0.4.
- Included all Sweetylisp files in source distribution.
...
- Thanks @mishan for your change to 'sweety plotnft show' to display Percent Successful Points.
...
## 1.2.2 Sweety blockchain 2021-07-13

### Fixed

- Converted test_rom.py to use pytest and fixed test_singleton.
- Thanks to @yshklarov for help fixing [#7273](https://github.com/Sweety-Network/sweety-blockchain/issues/7273), which bundled CA store to support pools for some farming systems, including M1 Apple computers. This enables those machines to properly connect to pools, and fixes the issue.

## 1.2.1 Sweety blockchain 2021-07-12
...
## 1.2.0 Sweety blockchain 2021-07-07

### Added

- Portable pooled plots are now available using our new plot NFT. These allow you to plot new plots to an NFT that can either self farm or join and leave pools. During development there were changes to the plot NFT so portable pool plots (those made with `-c` option to `sweety plots create`) using code from before June 25th are invalid on mainnet.
OG plots made before this release can continue to be farmed side by side with the new portable pool plots but can not join pools using the official pooling protocol. You can learn more as a farmer by checking out the [pool user guide](https://github.com/Sweety-Network/sweety-blockchain/wiki/Pooling-User-Guide). Pool operators and those wanting to understand how the official pooling protocol operates should check out our [pooling implementation reference repository](https://github.com/Sweety-Network/pool-reference). If you plan to use plot NFT, all your farmers and harvesters must be on 1.2.0 to function properly for portable pool plots.
...
- `sweety farm summary` and the GUI now use a new RPC endpoint to properly show plots for local and remote harvesters. This should address issues #6563, #5881, #3875, #1461.
- `sweety configure` now supports command line updates to peer count and target peer count.
...
- All Sweety repositories now automatically build M1 wheels and create a new MacOS M1 native installer.
- New CLI command `sweety plotnft` to manage pools.
...
- We updated sweetypos to version 1.0.3. This adds parallel reads to GetFullProof. Thanks to @marcoabreu ! We now print target/final directory early in the logs refs and log process ID. Thanks to @grayfallstown ! We are now using Gulrak 1.5.6.
@683280 optimized code in phase1.hpp. @jespino and @mrhacky started migrating to flags instead of booleans parameters for `show_progress` and `nobitfield`. If you are providing third-party tools you may need to make adjustments if relying on the sweetypos log.
- Updated sweetyvdf to version 1.0.2 to fix certain tests.
...
- We have increased the strictness of validating Sweetylisp in the mempool and clvm.
...
- Correctly display private key in `sweety keys show`.
- Thanks to @gldecurtins for removing a default printout of the private key mnemonic in `sweety keys show`.
...
- Thank you to @SweetyMineJP for various improvements.
...
- `sweety farm summary` better handles wallet errors.
...
### 1.1.7 Sweety Blockchain 2021-06-05
...
## 1.1.6 Sweety Blockchain 2021-05-20
...
- We added a simple profiler to track performance times in the application (see /sweety/util/profiler.py for instructions).
...
- Added configuration options for moving an install to testnet and back (use `sweety configure -t true|false`).
...
- We now use version 1.0.2 of sweetypos. We now reopen files on read or write failure with thanks to @mreid-moz! We also prevent sweetypos prover crashing for more bad plots.
...
## 1.1.5 Sweety Blockchain 2021-05-09
...
## 1.1.4 Sweety Blockchain 2021-05-04

### Changed

- Secret wallet key is hidden by default in `sweety keys show`. Use `sweety keys show --show-mnemonic-seed` for private keys.
...
## 1.1.3 Sweety Blockchain 2021-05-01
...
- We changed sweetylisp singletons to take a puzzlehash as its origin. We also updated the DID wallet to use this.
...
- Sweetylisp programs now remained serialized in Node for better performance.
...
## 1.1.2 Sweety Blockchain 2021-04-24
...
## 1.1.1 Sweety Blockchain 2021-04-21
...
## 1.1.0 Sweety Blockchain 2021-04-21

### Added

- This fork release includes full transaction support for the Sweety Blockchain. Transactions are still disabled until 5/3/2021 at 10:00AM PDT. It is hard to overstate how much work and clean up went into this release.
- This is the 1.0 release of Sweetylisp. Much has been massaged and finalized. We will be putting a focus on updating and expanding the documentation on [sweetylisp.com](https://sweetylisp.com) shortly.
...
## 1.0.5 Sweety Blockchain 2021-04-14
...
## 1.0.4 Sweety Blockchain 2021-04-12
...
- The `src` directory in sweety-blockchain has been changed to `sweety` to avoid namespace collisions.
- GUI install builds have been simplified to rely on one `.spec` file in `sweety/`
...
## 1.0.3 Sweety Blockchain 2021-03-30

### Added

- This is a minor bug fix release for version 1.0.2
- You should review the [release notes for v1.0.2](https://github.com/Sweety-Network/sweety-blockchain/releases/tag/1.0.2) but we especially want to point out that wallet sync is much faster than in 1.0.1 and earlier versions.

### Fixed

- An incorrect merge brought in unreleased features and broke `sweety keys`.
- Omitted from the 1.0.2 changelog, we fixed one crash in harvester with the release of sweetypos 1.0.0 as well.

## 1.0.2 Sweety Blockchain 2021-03-30

### Added

- We have released version 1.0.0 of [sweetypos](https://github.com/Sweety-Network/sweetypos). This includes a 20% speed increase for bitfield plotting compared to the previous version on the same machine. In many cases this will mean that bitfield plotting is as fast or faster than non bitfield plotting.
...
- We have added Dutch and Polish to supported translations. Thanks @psydafke, @WesleyVH, @pieterhauwaerts, @bartlomiej.tokarzewski, @abstruso, @feel.the.code, and @Axadiw for contributions to [translations on Crowdin](https://crowdin.com/project/sweety-blockchain).
...
- @jespino changed `sweety keys show` to require the `--show-mnemonic-seed` before it displays your 24 work private key mnemonic.
- We decreased the size of the block cache in node to perform better with longer chains.
- You can now add a private key mnemonic from a file with `sweety keys show`.
...
## 1.0.1 Sweety Blockchain 2021-03-23

### Added

- There is now a simple progress bar on the GUI Plot page and when you view the log from the three dots on the right.
- Users must now explicitly set the `--show-mnemonic-seed` flag to see their private keys when running `sweety keys show`.
- We are now building Linux GUI installers. These should be considered beta quality for now.
- Translations now available for German, Traditional Chinese, and Danish. Thanks to @Dravenex, @MaestroOnICe, @loudsyncro, @loppefaaret, @thirteenthd, @wong8888, @N418, and @swjz for all the translation help. You to can translate at our [Crowdin project](https://crowdin.com/project/sweety-blockchain/).
...
- Users can now pip install e.g. sweety-blockchain==1.0.1 on most platforms.
- Sometimes the GUI had an error regarding MainWindow.

## 1.0.0 First Release of Sweety Blockchain 2021-03-17

### Added

- This is the first production release of the Sweety Blockchain. This can be installed and will wait for the green flag that will be dropped at approximately 7AM PDST (14:00 UTC) on Friday March 19, 2021. All farming rewards from that point forward will be considered valid and valuable STY. There is a six week lock on all transactions. During those six weeks farmers will be earning their farming rewards but those rewards can not be spent.
...
- Thank you to @L3Sota for adding a Japanese translation via our [Crowdin project](https://crowdin.com/project/sweety-blockchain).
...
- Mainnet uses port 10010 and other constants and service names were changed for mainnet.
...
- Migration with SWEETY_ROOT set does not crash sweety init

## 1.0rc8 aka Release Candidate 8 - 2021-03-15

### Added

- This is a hard fork/breaking change from RC6/7. TSTY Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.sweety.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `sweety farm summary` will now display TSTY or STY as appropriate.
...
- `sweety farm summary` displayed some incorrect amounts.
...
- `sweety netspace` now defaults to 1000 blocks to mirror the GUI.
...
- Remove `sweety plots "-s" "--stripe_size"` and the strip size setting in the Advanced section of the GUI. We now always use the best default of 64K for the GUI and cli.
- `sweety keys add` takes secret words a prompt on the command line or stdin instead of command line arguments for security.
- Version 1.0.1 of sweetyvdf was added. This brought MPIR on Windows to the most recent release. Additionally we removed inefficient ConvertIntegerToBytes() and ConvertBytesToInt() functions, use GMP library's mpz_export/mpz_import for big integers and simple helper functions for built-in integer types. The latter are taken from sweetyvdf. We now require compressed forms to be encoded canonically when deserializing. This should prevent potential grinding attacks where some non-canonical encodings of a compressed form could be used to change its hash and thus the next challenges derived from it. Canonically encoded compressed forms must be reduced and must produce the same string when deserialized and serialized again.
...
- We now hand build Apple Silicon native binary wheels for all sweety-blockchain dependencies and host them at [https://pypi.sweety.net/simple](https://pypi.sweety.net/simple). We are likely to hand build a MacOS ARM64 dmg available and certainly will for 1.0. You can install natively on M1 now with the `git clone` developer method today. Just make sure Python 3.9 is installed. `python3 --version` works.
...
- Generators have been re-worked in CLVM. We added a sweetylisp deserialization puzzle and improved the low-level generator. We reduce the accepted atom size to 1MB during SweetyLisp native deserialization.
...
- We have moved to a new and much higher scalability download.sweety.net to support the mainnet launch flag and additional download demand.
- To always get the latest testnet and then mainnet installers you can now use a latest URL: [Windows](https://download.sweety.net/latest/Setup-Win64.exe) and [MacOS x86_64](https://download.sweety.net/latest/Setup-MacOS.dmg).
- Sweety wheels not on Pypi and some dependecies not found there also are now on pypi.sweety.net.
...
- PRs and commits to the sweety-blockchain-gui repository will automatically have their locales updated.

## Fixed

- The Farm page will now no longer get stuck at 50 TSTY farmed.
- `sweety farm` has had multiple bugs and spelling issues addressed. Thanks to @alfonsoperez, @soulmerge and @olivernyc for your contributions.
- `sweety wallet` had various bugs.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `sty_target_address` sections of config.yaml.
...
- The RC5 release is a new breaking change/hard fork blockchain. Plots and keys from previous chains will work fine on RC5 but balances of TSTY will not come forward.
- We now support a "green flag" chain launch process. A new version of the software will poll download.sweety.net/notify/ for a signed json file that will be the genesis block of the chain for that version. This will allow unattended start at mainnet.
...
- Thanks to @jespino there is now `sweety farm summary`. You can now get almost exactly the same farming information on the CLI as the GUI.
- We have added Romanian to the GUI translations. Thank you to @bicilis on [Crowdin](https://crowdin.com/project/sweety-blockchain). We also added a couple of additional target languages. Klingon anyone?
- `sweety wallet` now takes get_address to get a new wallet receive address from the CLI.
- `sweety plots check` will list out all the failed plot filenames at the end of the report. Thanks for the PR go to @eFishCent.
- Sweetylisp and the clvm have had the standard puzzle updated and we replaced `((c P A))` with `(a P A)`.
...
- Thanks to @xdustinface of Dash, the BlS Signature library has been updated to 0.9 with clean ups and some speed ups. This changed how the G2 infinity element was handled and we now manage it inside of sweety-blockchain, etc., instead of in blspy.
...
- In sweetyvdf we changed n-Wesolowski proofs to include B instead of y in segments. Proof segments now have the form (iters, B, proof) instead of (iters, y, proof). This reduces proof segment size from 208 to 141 bytes.
- The new sweetyvdf proof format is not compatible with the old one, however zero-Wesolowski proofs are not affected as they have zero proof segments and consist only of (y, proof).
- We made two HashPrime optimizations in sweetyvdf. This forces numbers being tested for primality to be odd and avoids an unnecessary update of the sprout vector by stopping after the first non-zero value. This is a breaking change as it changes the prime numbers generated from a given seed. We believe this is the final breaking change for sweetyvdf.
- sweetybip158 was set to a gold 1.0 version.
- Comments to Sweetylisp and clvm source have been updated for all of the Sweetylisp changes over the proceeding three weeks.
- And thanks yet again to @jespino for a host of PRs to add more detailed typing to various components in sweety-blockchain.
- aiohttp was updated to 3.7.4 to address a low severity [security issue](https://github.com/advisories/GHSA-v6wp-4m6f-gcjg).
- calccrypto/uint128_t was updated in the Windows sweetypos implementation. Sweetypos required some changes its build process to support MacOS ARM64.
...
- Adding Click changed the behavior of `sweety keys add -m`. The help now makes it clear that the 24 word mnemonic needs to be surrounded by a pair of quotes.
- Python root CA certificates have issues so we have added the Mozilla certificate store via curl.se and use that to connect to backup.sweety.net via https, for example.
- The difficulty adjustment calculation was simplified.
- All of the sweety sub repositories that were attempting to build MacOS Universal wheels were only generating x86_64 wheels internally. We have moved back to only generating x86_64 MacOS wheels on CI.
- However, we have updated and test compiled all Sweety dependencies on Apple Silicon and will be making available a test .dmg for MacOS ARM64 shortly.
- Various weight proof edge cases have been fixed.
- Various typos and style clean ups were made to the Click CLI implementation. `sweety -upnp f` was added to disable uPnP.
- `sweety plots check` shouldn't crash when encountering plots that cause RuntimeError. PR again thanks to @eFishCent.
...
- RC3 is a new chain to support the last major sweetylisp changes. TSTY from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- A new RPC api: /push_tx. Using this RPC, you can spend custom sweetylisp programs. You need to make a SpendBundle, which includes the puzzle reveal (sweetylisp), a solution (sweetylisp) and a signature.
- You can now use the RPC apis to query the mempool.
- There are now Swedish, Spanish, and Slovak translations. Huge thanks to @ordtrogen (Swedish), @jespino and @dvd101x (Spanish), and our own @seeden (Slovak). Also thanks were due to @f00b4r (Finnish), @A-Caccese (Italian), and @Bibop182 and @LeonidShamis (Russian). Quite a few more are almost complete and ready for inclusion. You can help translate and review translations at our [crowdin project](https://crowdin.com/project/sweety-blockchain).
- You can obtain a new wallet receive address on the command line with `sweety wallet new_address`. Thanks to @jespino for this and a lot more in the next section below.
- You will now see Your Harvester Network in the GUI even if you have no plots.

### Changed

- All sweetylisp opcodes have been renumbered. This should be the last major breaking change for sweetylisp and the clvm. There are a couple minor enhancements still needed for mainnet launch, but they may or may not require minor breaking changes. We will be restarting testnet chains on a mostly weekly basis either way.
- Node batch syncing performance was increased, and it now avoids re-validating blocks that node had already validated.
- The entire CLI has been ported to [Click](https://click.palletsprojects.com/en/7.x/). Huge thanks to @jespino for the big assist and @unparalleled-js for the [recommendation and the initial start](https://github.com/Sweety-Network/sweety-blockchain/issues/464). This will make building out the CLI much easier. There are some subtle changes and some shortcuts are not there anymore. `sweety -h` and `sweety SUBCOMMAND -h` can be your guide.
...
- `sweety plots check` now will continue checking after it finds an error in a plot to the total number of checks you specified.
...
- Interface improvements for `sweety netspace`.
- Now that aiosqlite included our upstream improvements we install version 0.17.0.
- `sweety init` only migrates release candidate directories. The versioned sub directories under `~/sweety` will be going away before mainnet.
...
- `sweety configure -log-level` was broken.
...
- This is an errata release for Release Candidate 1. There were a couple of things that did not smoothly migrate from the Beta versions. Please make sure you also consult the [release notes for RC-1](https://github.com/Sweety-Network/sweety-blockchain/releases/tag/1.0rc1) was well.
...
- This is the first release in our release candidate series. There are still a few things that will change at the edges but the blockchain, clvm, and sweetylisp are in release form. We have one major change to sweetylisp/clvm that we have chosen to schedule for the next release as in this release we're breaking the way q/quote works. We also have one more revision to the VDF that will decrease the sizes of the proofs of time. We expect a few more releases in the release candidate series.
- Installers will now be of the pattern SweetySetup-0.2.1.exe. `0.2` is release candidate and the final `.1` is the first release candidate.
- Use 'sweety wallet get_transactions' in the command line to see your transactions.
- 'sweety wallet show' now shows your wallet's height.
...
- We have integrated with crowdin to make it easier to translate the GUI. Check out [Sweety Blockchain GUI](https://crowdin.com/project/sweety-blockchain) there.
- We have added Italian, Russian, and Finnish. More to come soon.
- There is now remote UI support. [Documents](https://github.com/Sweety-Network/sweety-blockchain-gui/blob/main/remote.md) will temporarily live in the repository but have moved to the [wiki](https://github.com/Sweety-Network/sweety-blockchain/wiki/Connecting-the-UI-to-a-remote-daemon). Thanks to @dkackman for this excellent addition!
...
- Added `sweety plots check --challenge-start [start]` that begins at a different `[start]` for `-n [challenges]`. Useful when you want to do more detailed checks on plots without restarting from lower challenge values you already have done. Huge thanks to @eFishCent for this and all of the debugging work behind the scenes confirming that plot failures were machine errors and not bugs!
...
- 'sweety wallet send' now takes in TSTY or STY as units instead of mojos.
...
- We updated sweetypos to hopefully address some harvester crashes when moving plot files.
...
- The Beta 27 chain is a hard fork. All TSTY from previous releases has been reset on this chain. Your keys and plots of k=32 or larger continue to work just fine on this new chain.
...
- You can now send and receive transactions with the command line. Try `sweety wallet -h` to learn more. Also, `sweety wallet` now requires a third argument of `show`, therefor you will use `sweety wallet show` to see your wallet balance.
- We have added the [Crowdin](https://crowdin.com/) translation platform to [sweety blockchain gui](https://crowdin.com/project/sweety-blockchain). We are still getting it fully set up, but helping to translate the GUI is going to be much easier.
...
- `sweety init -c [directory]` will create new TLS certificates signed by your CA located in `[directory]`. Use this feature to configure a new remote harvester. Type `sweety init -h` to get instructions. Huge thanks to a very efficient @eFishCent for this quick and thorough pull request.
- We build both MacOS x86_64 and MacOS universal wheels for sweetypos, sweetyvdf, blpsy, and sweetybip158 in Python 3.9. The universal build allows M1 Macs to run these dependencies in ARM64 native mode.
...
- This release has the final mainnet rewards schedule. During the first three years, each block winner will win 2 TSTY/STY per block for a total of 9216 TSTY per day from 4608 challenges per day.
- Smart transactions now use an announcement instead of 'coin consumed' or lock methods.
- The GUI is now in a separate submodule repository from sweety-blockchain, [sweety-blockchain-gui](https://github.com/Sweety-Network/sweety-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `sweety-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
...
- The cli wallet, `sweety wallet`, was incorrectly displaying TSTY balances as if they were Coloured Coins.
...
- `sweety plots check -l` will list all duplicate plot IDs found on the machine. Thanks very much for this PR @eFishCent.

### Changed

- Significant improvements have been made to how the full node handles the mempool. This generally cuts CPU usage of node by 2x or more. Part of this increase is that we have temporarily limited the size of transactions. If you want to test sending a transaction you should keep the value of your transaction below 20 TSTY as new consensus will cause you to use a lot of inputs. This will be returned to the expected level as soon as the integration of [clvm rust](https://github.com/Sweety-Network/clvm_rs) is complete.
- We have changed the way TLS between nodes and between sweety services work. Each node now has two certificate authorities. One is a public, shared CA that signs the TLS certificates that every node uses to connect to other nodes on 10010 or 510010. You now also have a self generated private CA that must sign e.g. farmer and harvester's certificates. To run a remote harvester you need a new harvester key that is then signed by your private CA. We know this is not easy for remote harvester in this release but will address it quickly.
...
- We have made the "Help Translate" button in the GUI open in your default web browser and added instructions for adding new translations and more phrases in existing translations at that [URL](https://github.com/Sweety-Network/sweety-blockchain/tree/main/electron-react/src/locales). Try the "Help Translate" option on the language selection pull down to the left of the dark/light mode selection at the top right of the GUI.
- Sync store now tracks all connected peers and removes them as they get removed.
- The Rate Limited Wallet has been ported to new consensus and updated Sweetylisp methods.
- We are down to only one sub dependency that does not ship binary wheels for all four platforms. The only platform still impacted is ARM64 (generally Raspberry Pi) but that only means that you still need the minor build tools as outlined on the [wiki](https://github.com/Sweety-Network/sweety-blockchain/wiki/Raspberry-Pi).
...
- Last Attempted Proof is now fixed. This will show you the last time one of your plots passed the [plot filter](https://github.com/Sweety-Network/sweety-blockchain/wiki/FAQ#what-is-the-plot-filter-and-why-didnt-my-plot-pass-it).
...
- There was a potential bug in Python 3.9.0 that required us to update blspy, sweetypos, sweetyvdf, and sweetybip158.
...
- `sweety plots check` enforces a minimum of `-n 5` to decrease false negatives. Thanks to @eFishCent for these ongoing pull requests!
...
- clvm bytecode is now generated and confirmed that the checked-in clvm and SweetyLisp code matches the CI compiled code.
- We have removed the '-r' flag from `sweety` as it was being overridden in most cases by the `-r` for restart flag to `sweety start`. Use `sweety --root-path` instead.
- `sweety -h` now recommends `sweety netspace -d 192` which is approximately one hours worth of sub blocks. Use `-d 1000` to get the same estimate of netspace as the RPC and GUI.
- `sweety show -c` now displays in MiB and the GUI has been changed to MiB to match.
- `sweety configure` now accepts the shorter `-upnp` and `-log-level` arguments also.
- `sweety plots check` now defaults to `-n 30` instead of `-n 1` - HT @eFishCent.
- `sweety plots create` now enforces a minimum of k=22. As a reminder, anything less than k=32 is just for testing and be careful extrapolating performance of a k less than 30 to a k=32 or larger.
...
- `sweety show -c` was showing upload MiB in the download column and vice versa. @pyl and @psydafke deserves credit for insisting it was broken and @kd637xx for the PR assist.
- `sweety show` handles sub block 0 better.
...
- `sweety plots create -x` skips adding [final dir] to harvester for farming
...
- Removed loading plot file names when starting `sweety plots create`; decreases plotter time when there are a lot of plots on the machine. Huge thanks to @eFishCent for this PR!
...
- `sweety show -h` now correctly states that you use the first 8 characters of the node id to remove a node on the cli.
- Thank you to @wallentx for adding better help for `sweety configure --enable-upnp`.
...
- `sweety configure` now lets you enable or disable uPnP.
...
- `sweety netspace -s` would not correctly look up the start block height by block hash. Additionally netspace now flips to PiB above 1024 TiB. To compare netspace to `sweety show` of the GUI use `sweety netspace -d 1000` as `sweety netspace` defaults to `-d 192` which is one hour.
...
- Welcome to the new consensus. This release is an all but a full re-write of the blockchain in under 30 days. There is now only one tip of the blockchain but we went from two chains to three. Block times are now a little under a minute but there are a couple of sub blocks between each transaction block. A block is also itself a special kind of sub block and each sub block rewards the farmer who won it 1 TSTY. Sub blocks come, on average, about every 17 to 18 seconds.
- Starting with this Beta, there are 4608 opportunities per day for a farmer to win 1 TSTY compared to Beta 18 where there were 288 opportunities per day for a farmer to win 16 TSTY.
- There is a lot more information and explanation of the new consensus algorithm in the New Consensus Working Document linked from [sweety.net](https://sweety.net/). Among the improvements this gives the Sweety blockchain are a much higher security level against all attacks, more frequent transaction blocks that have less time variation between them and are then buried under confirmations (sub blocks also count towards re-org security) much more quickly.
- New consensus means this is a very hard fork. All of your TSTY from Beta 17/18 will be gone. Your plots and keys will work just fine however. You will have to sync to the new chain.
...
- Prior to this Beta 19, we had block times that targeted 5 minutes and rewarded 16 TSTY to one farmer. Moving forward we have epoch times that target 10 minutes and reward 32 TSTY to 32 farmers about every 17-18 seconds over that period. This has subtle naming and UI impacts in various places.
...
- You can now specify which private key to use for `sweety plots create`. After obtaining the fingerprint from `sweety keys show`, try `sweety plots create -a FINGERPRINT`. Thanks to @eFishCent for this pull request!
- We use a faster hash to prime function for sweetyvdf from the current release of gmp-6.2.1 which we have upgraded sweetyvdf and blspy to support.
- There is a new cli command - `sweety configure`. This allows you to update certain configuration details like log level in config.yaml from the command line. This is particularly useful in containerization and linux automation. Try `sweety configure -h`. Note that if sweety services are running and you issue this command you will have to restart them for changes to take effect but you can use this command in the venv when no services are running or call it directly by path in the venv without activating the venv. Expect the options for this command to expand.
...
- The Farm tab in the GUI is significantly enhanced. Here you have a dashboard overview of your farm and your activity in response to challenges blockchain challnegs, how long it will take you - on average - to win a block, and how much TSTY you've won so far. Harvester and Full Node connections have moved to Advanced Options.
- Harvester and farmer will start when the GUI starts instead of waiting for key selection if there are already keys available. This means you will start farming on reboot if you have the Sweety application set to launch on start.
- Testnet is now running at the primary port of 510010. Update your routers appropriately. This opens 10010 for mainnet.
- All networking code has been refactored and mostly moved to websockets.
- RPCs and daemon now communicate over TLS with certificates that are generated into `~/.sweety/VERSION/config/`
...
- All appropriate Sweetylisp smart transactions have been updated to use aggsig_me.
...
- sweetyvdf now allows passing in input to a VDF for new consensus.
- sha256tree has been removed from Sweetylisp.
- `sweety show -s` has been refactored to support the new consensus.
- `sweety netspace` has been refactored for new consensus.
...
- Tests now place a cache of blocks and plots in the ~/.sweety/ directory to speed up total testing time.
- Changes were made to sweetypos to correctly support the new bitfiled backpropogation on FreeBSD and OpenBSD. With the exception of needing to work around python cryptography as outlined on the wiki, FreeBSD and OpenBSD should be able to compile and run sweety-blockchain.
...
- `sweety show -s` now is less brain dead when a node is initially starting to sync.
...
- We have bitfield optimized phase 2 of plotting. There is only about a 1% increase in speed from this change but there is a 12% decrease in writes with a penalty of 3% more reads. More details in [PR 120](https://github.com/Sweety-Network/sweetypos/pull/120). Note that some sorts in phase 2 and phase 3 will now appear "out of order" and that is now expected behavior.
- Partial support for Python 3.9. That includes new versions of Sweety dependencies like sweetybip158.
...
- A segfault caused by memory leaks in bls-library has been fixed. This should end the random farmer and harvester crashes over time as outlined in [Issue 500](https://github.com/Sweety-Network/sweety-blockchain/issues/500).
...
- FreeBSD and OpenBSD should be able to build and install sweety-blockchain and its dependencies again.
...
- In the GUI there was [a regression](https://github.com/Sweety-Network/sweety-blockchain/issues/484) that removed the scroll bar on the Plot page. The scroll bar has returned!
...
- Logging had [a regression](https://github.com/Sweety-Network/sweety-blockchain/issues/485) where log level could no longer be set by service.

## [1.0beta16] aka Beta 1.16 - 2020-10-20

### Added

- The Sweety GUI now supports dark and light mode.
- The GUI now supports translations and localizations. If you'd like to add your language you can see the examples in [the locales directory](https://github.com/Sweety-Network/sweety-blockchain/tree/dev/electron-react/src/locales) of the sweety-blockchain repository.
- `sweety check plots` now takes a `-g` option that allows you to specify a matching path string to only check a single plot file, a wild card list of plot files, or all plots in a single directory instead of the default behavior of checking every directory listed in your config.yaml. A big thank you to @eFishCent for this pull request!
...
- Updated to sweetypos 0.12.32. This update significantly speeds up the F1/first table plot generation. It also now can log disk usage while plotting and generate graphs. More details in the [sweetypos release notes](https://github.com/Sweety-Network/sweetypos/releases/tag/0.12.32).
...
- sweetypos 0.12.32 fixed a an out of bounds read that could crash the plotter. It also contains a fix to better handle the case of drive letters on Windows.
...
- sweetyvdf now has full IFMA optimizations for processors that support it.

### Changed

- Multithreading support in sweetypos, as well as a new algorithm which is faster and does 70% less IO. This is a significant improvement in speed, much lower total writing, and configurability for different hardware environments.
...
- The rpc interfaces of all sweety services have been refactored, simplified, and had various additional functionality added.
- Block timestamps are now stored in the wallet database. Both database versions were incremented and databases from previous versions will not work with Beta 14. However, upon re-sync all test sweety since Beta 12 should appear in your wallet.
...
- Sweetypos issue where memory was spiking and increasing
- Fixed working space estimates so they are exact
- Log all errors in sweetypos
...
- Long_description_content_type is now set to improve sweety-blocksweetyn's Pypi entry. Thanks to @altendky for this pull request.
...
- You can now backup your wallet related metadata in an encrypted and signed file to a free service from Sweety Network at backup.sweety.net. Simply having a backup of your private key will allow you to fully restore the state of your wallet including coloured coins, rate limited wallets, distributed identity wallets and many more. Your private key is used to automatically restore the last backup you saved to the Sweety backup cloud service. This service is open source and ultimately you will be able to configure your backups to go to backup.sweety.net, your own installation, or a third party's version of it.
- Added a Code of Conduct in CODE_OF_CONDUCT.md.
- Added a bug report template in `.github/ISSUE_TEMPLATE/bug_report.md`.

### Changed

- This is a new blockchain as we changed how the default puzzle hashes are generated and previous coins would not be easy to spend. Plots made with Beta 8 and newer continue to work, but all previous test sweety are left on the old chain and do not migrate over. Configuration data like plot directories automatically migrate in your `~/.sweety` directory.
...
- `sweety init` now automatically discovers previous releases in each new release.

### Fixed

- `sweety show -w` should now more reliably work. Wallet balances should be more often correct.
...
- Fixed a typo in `sweety show -w` with thanks to @pyl on Keybase.
- In Windows the start menu item is now Sweety Network and the icon in Add/Remove is updated.

## [1.0beta11] aka Beta 1.11 - 2020-08-24

### Added

- The Sweety UI now has a proper About menu entry that gives the various component versions and directs people to submit issues on GitHub. Thank you to @freddiecoleman for this pull request!
- Ability to run only the farmer, wallet, or timelord services, for more advanced configurations (sweety run farmer-only, wallet-only, timelord-only)
...
- `sweety show -w` now correctly outputs all wallet types and balances from the local wallet.
...
- `sweety show -b` now returns plot public key and pool public key for each block.
- Added cbor2 binary wheels for ARM64 to the Sweety simple site. Raspberry Pi should be just a little easier to install.

### Changed

- Wallet addresses and other key related elements are now expressed in Chech32 which is the Sweety implementation of [Bech32](https://github.com/bitcoin/bips/blob/master/bip-0173.mediawiki). All of your old wallet addresses will be replaced with the new Chech32 addresses. The only thing you can't do is send test sweety between 1.8/1.9 and 1.10 software. Anyone who upgrades to 1.10 will keep their transactions and balances of test sweety from the earlier two releases however.
...
- The proof of space document in sweetypos has been updated to the new format and edited for clarity. Additionally GitHub actions now has the on demand ability to create the PDF version.
...
- `sweety show -w` now displays a message when balances cannot be displayed instead of throwing an error. Thanks to @freddiecoleman for this fix!
- Fix issue with shutting down full node (full node processes remained open, and caused a spinner when launching Sweety)
- Various code review alerts for comparing to a wider type in sweetypos were fixed. Additionally, unused code was removed from sweetypos
...
- See wallet balances in command line: `sweety show -w`
- Retry opening invalid plots every 20 minutes (so you can copy a large plot into a plot directory.)
- We've added `sweety keys sign` and `sweety keys verify` to allow farmers to certify their ownership of keys.
...
- `sweety start wallet-server` changed to `sweety start wallet`, for consistency.
...
- Removed legacy scripts such as sweety-stop-server, sweety-restart-harvester, etc.
...
are read.
- Updated [blspy](https://github.com/Sweety-Network/bls-signatures) to use the
...
- The various sub repositories (sweetypos, sweetyvdf, etc.) now build ARM64 binary
...
- The sweety executable is now available if installing from the Windows or MacOS
Graphical installer. Try `./sweety -h` from
`~\AppData\Local\Sweety-Blockchain\app-0.1.8\resources\app.asar.unpacked\daemon\`
in Windows or
`/Applications/Sweety.app/Contents/Resources/app.asar.unpacked/daemon` on MacOS.

### Changed

- Minor changes have been made across the repositories to better support
compiling on OpenBSD. HT @n1000.
- Changed STY units to TSTY units for testnet.
...
"sweety plots create" and "sweety plots check".
- Add plot directories to config.yaml from the cli with "sweety plots add".
- Use real plot sizes in UI instead of a formula/
- HD keys now use EIP 2333 format instead of BIP32, for compatibility with
other chains.
- Keys are now derived with the EIP 2334 (m/12381/10010/a/b).
- Removed the ability to pass in sk_seed to plotting, to increase security.
- Linux builds of sweetyvdf and blspy now use a fresh build of gmp 6.2.1.
...
- The Windows installer is now signed by a Sweety Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- Test plots for the simulation and testing harness now go into `~/.sweety/test-plots/`
- We have completely refactored all networking code towards making each Sweety service use the same default networking infrastructure and move to websockets as the default networking wire protocol.
- We added additional improvements and more RPCs to the start daemon and various services to continue to make sweety start/stop reliable cross platform.
...
- We now codesign the Apple .dmg installer with the Sweety Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
...
- Starting sweety now waits for the full node to be active before contacting the introducer.
...
- You can now create or import private keys with a 24 word mnemonic, both in the UI and 'sweety keys' command line.
...
- We added a `sweety keys` command line program, to see, add, and remove private keys.
...
- We added [lgtm](https://lgtm.com/) source analysis on pull request to the sweety-blockchain, sweetypos, sweetyvdf, sweetybip158, and bls-library repositories to add some automated security analysis to our ci.
...
- `sweety netspace` has been refactored to use the `/get_network_space` RPC. The command
...
- sweetypos is now easier to compile with MSVC.
...
- `sweety-check-plots` uses the plot root and checks the plots in the same manner as harvester.
- `sweety-check-plots` now does not override plots.yaml, which means concurrent plots will properly be added to plots.yaml.
...
- We have added an option to `sweety-create-plots` to specify the second temporary directory. Creating a plot is a three step process. First a working file ending in `.dat.tmp` is created. This file is usually 5 times larger than the final plot file. In the later stages of plotting a second temp file is created ending in `.dat.2.tmp` which will grow to the size of the final plot file. In the final step, the `.dat.2.tmp` is copied to the final `.dat` plot file. You can now optionally set the directory for the `.dat.2.tmp` file with the `-2` flag. An example use case is plotting on a ramdisk and writing both the second temp file and the final file out to an SSD - `sweety-create-plots -n 1 -k 30 -t /mnt/ramdisk -2 /mnt/SSD -d /mnt/SSD`.

### Changed

- `sweety init` properly migrates from previous versions including the k>=32 workaround. Additionally, the farming target key is checked to make sure that it is the valid and correct public key format.
- We have implemented a workaround for the `sweety start` issues some were having upon crash or reboot. We will be rebuilding start and stop to be robust across platforms.
- This release re-includes `sweety-start-harvester`.
...
- The user interface now refers to sweety coins with their correct currency code of STY.
...
- Building the sub reposities (sweetypos, sweetyvdf, blslibrary) now requires CMake 3.14+.

### Fixed

- There was a regression in Sweety Proof of Space ([sweetypos](https://github.com/Sweety-Network/sweetypos)) that came from our efforts to speed up plotting on Windows native. Now k>=32 plots work correctly. We made additional bug fixes and corrected limiting small k size generation.
- There was a bug in Timelord handling that could stop all VDF progress.

### Deprecated

- We have made significant changes to the full node database to make it more reliable and quicker to restart. This requires re-syncing the current chain. If you use `sweety init` then sync on first start will happen automatically. "\$SWEETY_ROOT" users will need to delete `$SWEETY_ROOT/db/*` before starting Beta 1.5. This also fixes the simulation issue in Beta 1.4 where tips could go "back in time."
...
- Now you can run estimates of the total space currently farming the network. Try `sweety netspace -d 12` to run an estimate over the last 12 blocks which is approximately 1 hour.
- We’ve added TLS authentication for incoming farmer connections. TLS certs and keys are generated during sweety init and only full nodes with your keys will be able to connect to your Farmer. Also, Harvester, Timelord, and Wallet will now not accept incoming connections which reduces the application attack surface.
- The node RPC has a new endpoint get_header_by_height which allows you to retrieve the block header from a block height. Try `sweety show -bh 1000` to see the block header hash of block 1000. You can then look up the block details with `sweety show -b f655e1a9f7f8c89a703e40d9ce82ae33508badaf7b37fa1a56cad27926b5e936` which will look up a block by it's header hash.
...
- Most of the content of README.md and INSTALL.md have been moved to the [repository wiki](https://github.com/Sweety-Network/sweety-blockchain/wiki) and placed in [INSTALL](https://github.com/Sweety-Network/sweety-blockchain/wiki/INSTALL) and [Quick Start Guide](https://github.com/Sweety-Network/sweety-blockchain/wiki/Quick-Start-Guide)
...
- Most scripts have been removed in favor of sweety action commands. You can run `sweety version` or `sweety start node` for example. Just running `sweety` will show you more options. However `sweety-create-plots` continues to use the hyphenated form. Also it's now `sweety generate keys` as another example.
- Sweety start commands like `sweety start farmer` and `sweety stop node` now keep track of process IDs in a run/ directory in your configuration directory. `sweety stop` is unlikely to work on Windows native for now. If `sweety start -r node` doesn't work you can force the run/ directory to be reset with `sweety start -f node`.
- We suggest you take a look at our [Upgrading documentation](https://github.com/Sweety-Network/sweety-blockchain/wiki/Updating-beta-software) if you aren't performing a new install.
...
- The script `sweety start wallet-gui` has been chaned to `sweety start wallet` which launches but the GUI and server on MacOS and Linux. `sweety start wallet-server` remains for WSL 2 and Windows native.
...
- Plots of k>=32 are not working for farming, and some broken plots can cause a memory leak. A [workaround is available](https://github.com/Sweety-Network/sweety-blockchain/wiki/Beta-1.4-k=32-or-larger-work-around).
...
- Currently, there is no way to restore a Coloured Coin Wallet.
- The `sweety stop all` command sometimes fails, use `sweety-stop-all` instead. In windows, use the task manager to stop the servers.
...
- There is a new command `sweety init` that will migrate files and generate your initial configuration. If you want to use the Wallet or farm, you will also have to `sweety-generate-keys`. You can read step by step instructions for [upgrading from a previous beta release](https://github.com/Sweety-Network/sweety-blockchain/wiki/Updating-beta-software). If you've set `$SWEETY_ROOT` you will have to make sure your existing configuration remains compatible manually.
...
- We added full block lookup to `sweety show`.

### Changed

- `sweety-restart-harvester` has been renamed from `sweety-start-harvester` to better reflect its functionality. Use it to restart a harvester that's farming so that it will pick up newly finished plots.
...
- If you previously used the plot_root variable in config, your plot directory names might not migrate correctly. Please double check the filenames in `~/.sweety/beta-1.0b3/config/plots.yaml` after migrating
...
- There is now full transaction support on the Sweety blockchain. In this initial Beta 1.0 release, all transaction types are supported though the wallets and UIs currently only directly support basic transactions like coinbase rewards and sending coins while paying fees. UI support for our [smart transactions](https://github.com/Sweety-Network/wallets/blob/main/README.md) will be available in the UIs shortly.
...
- We have revamped the sweety management command line. To start a farmer all you have to do is start the venv with `. ./activate` and then type `sweety-start-farmer &`. The [README.md](https://github.com/Sweety-Network/sweety-blockchain/blob/main/README.md) has been updated to reflect the new commands.
...
- Configuration directories have been moved to a default location of HomeDirectory/.sweety/release/config, plots/ db/, wallet/ etc. This can be overridden by `export SWEETY_ROOT=~/.sweety` for example which would then put the plots directory in `HomeDirectory/.sweety/plots`.
- The libraries sweety-pos, sweety-fast-vdf, and sweety-bip-158 have been moved to their own repositories: [sweetypos](https://github.com/Sweety-Network/sweetypos), [sweetyvdf](https://github.com/Sweety-Network/sweetyvdf), and [chaibip158](https://github.com/Sweety-Network/sweetybip158). They are brought in by sweety-blockchain at install time. Our BLS signature library remains at [bls-signatures](https://github.com/Sweety-Network/bls-signatures).
- The install process now brings in sweetypos, sweetyvdf, etc from Pypi where they are auto published via GitHub Actions ci using cibuildwheel. Check out `.github/workflows/build.yml` for build methods in each of the sub repositories.
- `sweety-regenerate-keys` has been renamed `sweety-generate-keys`.
...
- `install-timelord.sh` has been pulled out from `install.sh`. This script downloads the source python package for sweetyvdf and compiles it locally for timelords. vdf_client can be included or excluded to make building normal user wheels easier.

### Removed

- The Beta release is not compatible with the history of the Alpha blockchain and we will be ceasing support of the Alpha chain approximately two weeks after the release of this Beta. However, your plots and keys are fully compatible with the Beta chain. Please save your plot keys! Examples of how to save your keys and upgrade to the Beta are available on the [repo wiki](https://github.com/Sweety-Network/sweety-blockchain/wiki).
- The ssh ui and web ui are removed in favor of the cli ui and the Electron GUI. To mimic the ssh ui try `sweety show -s -c` and try `sweety show --help` for usage instructions.
...
- New AJAX based full node UI. To access go to [http://127.0.0.1:10011/index.html](http://127.0.0.1:10011/index.html) with any modern web browser on the same machine as the full node.
- If you want to benchmark your CPU as a VDF you can use vdf_bench square_asm 500000 for the assembly optimized test or just vdf_bench square 500000 for the plain C++ code path. This tool is found in lib/sweetyvdf/fast_vdf/.
...
- Sweety fullnode, farmer and harvester now default to logging to sweety.log in the sweety-blockchain directory. This is configured in config.yaml and due to config.yaml changes it is recommended to edit the new template config instead of using older config.yaml’s from previous versions.
...
- VDF improvements - VDF verification and generation speed has increased and dependence on flint2 has been removed. We wish to thank Dr. William Hart (@wbhart) for dual licensing parts of his contributions in FLINT and Antic for inclusion in the Sweety blockchain.
- Implemented an RPC interface with JSON serialization for streamables - currently on port 10011.
- Added details on how to contribute in CONTRIBUTING.md. Thanks @RichardLitt.
- Added color logging
- Now sweety_harvester will periodically announce which plots it is currently farming and their k sizes.

### Changed

- Moved the ssh UI to use RPC.
- Changed the displayed process names for harvester, farmer, fullnode, timelords, and VDFs to to sweety_full node, sweety_harvester, etc. Fixed a bug that could cause inadvertent shutdown of other processes like an ongoing plotting session when new sweety services were started.
...
- Introducer now makes sure it only sends peer addresses to peers of peers that it can reach on port 10010 or their UPnP port.
...
- This is the first release of the Sweety testnet! Blockchain consensus, proof of time, and proof of space are included.
- More details on the release at [https://www.sweety.net/developer/](https://www.sweety.net/developer/)

[unreleased]: https://github.com/Sweety-Network/sweety-blockchain/compare/1.0beta5...dev
[1.0beta5]: https://github.com/Sweety-Network/sweety-blockchain/compare/1.0beta4...1.0beta5
[1.0beta4]: https://github.com/Sweety-Network/sweety-blockchain/compare/1.0beta3...1.0beta4
[1.0beta3]: https://github.com/Sweety-Network/sweety-blockchain/compare/1.0beta2...1.0beta3
[1.0beta2]: https://github.com/Sweety-Network/sweety-blockchain/compare/1.0beta1...1.0beta2
[1.0beta1]: https://github.com/Sweety-Network/sweety-blockchain/compare/alpha-1.5.1...1.0beta1
[alpha 1.5.1]: https://github.com/Sweety-Network/sweety-blockchain/compare/alpha-1.5...alpha-1.5.1
[alpha 1.5]: https://github.com/Sweety-Network/sweety-blockchain/compare/alpha-1.4.1...alpha-1.5
[alpha 1.4.1]: https://github.com/Sweety-Network/sweety-blockchain/compare/alpha-1.4...alpha-1.4.1
[alpha 1.4]: https://github.com/Sweety-Network/sweety-blockchain/compare/alpha-1.3...alpha-1.4
[alpha 1.3]: https://github.com/Sweety-Network/sweety-blockchain/compare/alpha-1.2...alpha-1.3
[alpha 1.2]: https://github.com/Sweety-Network/sweety-blockchain/compare/alpha-1.1.1...alpha-1.2
[alpha 1.1.1]: https://github.com/Sweety-Network/sweety-blockchain/compare/alpha-1.1...alpha-1.1.1
[alpha 1.1]: https://github.com/Sweety-Network/sweety-blockchain/compare/alpha-1.0...alpha-1.1
[alpha 1.0]: https://github.com/Sweety-Network/sweety-blockchain/releases/tag/Alpha-1.0
