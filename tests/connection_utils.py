...
async def disconnect_all_and_reconnect(server: SweetyServer, reconnect_to: SweetyServer) -> bool:
...
async def add_dummy_connection(
    server: SweetyServer, dummy_port: int, type: NodeType = NodeType.FULL_NODE
    ...
    generate_ca_signed_cert(
        server.sweety_ca_crt_path.read_bytes(), server.sweety_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.sweety_ca_crt_path, server.sweety_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: SweetyServer, server_2: SweetyServer) -> WSSweetyConnection:
    """
