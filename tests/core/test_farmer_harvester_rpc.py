...
from sweetypos import DiskPlotter

from sweety.consensus.coinbase import create_puzzlehash_for_pk
from sweety.plotting.util import stream_plot_info_ph, stream_plot_info_pk, PlotRefreshResult
from sweety.plotting.manager import PlotManager
from sweety.protocols import farmer_protocol
...
from sweety.rpc.harvester_rpc_client import HarvesterRpcClient
...
from sweety.wallet.derive_keys import master_sk_to_wallet_sk, master_sk_to_pooling_authentication_sk
...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            ...
            await client.set_reward_targets(encode_puzzle_hash(new_ph, "sty"), encode_puzzle_hash(new_ph_2, "sty"))
            ...
            await client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "sty"))
            ...
            assert config["farmer"]["sty_target_address"] == encode_puzzle_hash(new_ph, "sty")
            assert config["pool"]["sty_target_address"] == encode_puzzle_hash(new_ph_3, "sty")

            new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "sty")
